...
const xmlfile = fs.readFileSync('nfe-example.xml', 'utf8');
const options = { compact: true, ignoreComment: true, spaces: 4 };

const { nfeProc } = convert.xml2js(xmlfile, options);

const {NFe} = nfeProc;
console.log(NFe);
return;

const { declaration, NOTAS_FISCAIS } = convert.xml2js(xmlfile, options);
...
function nfseXmlParser(nfse) {
  ...
  const response = {
    ...
    prestador: {
      ...
      insc_municipal: nfse.PRESTADOR_INSCRICAO_MUNICIPAL
        ? nfse.PRESTADOR_INSCRICAO_MUNICIPAL._text
        : null,
      razao_social: nfse.PRESTADOR_RAZAO_SOCIAL
        ? nfse.PRESTADOR_RAZAO_SOCIAL._text
        : null,
      nome_fantasia: nfse.PRESTADOR_NOME_FANTASIA
        ? nfse.PRESTADOR_NOME_FANTASIA._text
        : null,
      logradouro: nfse.PRESTADOR_LOGRADOURO._text,
      numero: nfse.PRESTADOR_PREST_NUMERO._text,
      complemento: nfse.PRESTADOR_COMPLEMENTO
        ? nfse.PRESTADOR_COMPLEMENTO._text
        : null,
      ...
      ddd: nfse.PRESTADOR_DDD_TELEFONE
        ? nfse.PRESTADOR_DDD_TELEFONE._text
        : null,
    ...
    tomador: {
      ...
      complemento: nfse.TOMADOR_COMPLEMENTO
        ? nfse.TOMADOR_COMPLEMENTO._text
        : null,
...
console.log('----------------- FIM ------------------');
console.log(
  `${nfse_prestadas.length} Notas Fiscais de Serviços Prestados e ${nfse_tomadas.length} Notas Fiscais de Serviços Tomados.`
);
